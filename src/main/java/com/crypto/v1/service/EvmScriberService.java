...
//        // 获取发送方地址的 nonce 值
//        BigInteger nonce = Web3Until.getNonce(web3j,owner);
//        System.out.println("nonce: " + nonce);
        BigInteger nonce =new BigInteger("2018");

                BigInteger gasPrice = Web3Until.getGasPrice(web3j);
//        BigInteger gasPrice = new BigInteger("120000000");
        ...
        int chainId = Web3Until.getChainId(web3j);
...
//        // 获取发送方地址的 nonce 值
//        final BigInteger[] nonce = {Web3Until.getNonce(web3j, owner)};
//        System.out.println("nonce: " + nonce[0]);
        final BigInteger[] nonce = {new BigInteger("2426")};
        ...
        for (int i =0;i<200;i++) {
            sleep(3);
                RawTransaction rawTransaction = RawTransaction.createTransaction(
                        ...
                        BigInteger.valueOf(21896), toAddress,
                        inputData);
                byte[] signedMessage = TransactionEncoder.signMessage(rawTransaction, chainId, credentials);
//                EthSendTransaction ethSendTransaction = web3j.ethSendRawTransaction(
//                        Numeric.toHexString(signedMessage)).send();
